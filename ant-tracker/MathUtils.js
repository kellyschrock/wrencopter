...
function moveLocation(origin, heading, distance) {
    const p = newCoordFromBearingAndDistance(origin, heading, distance);
    origin.lat = p.lat;
    origin.lng = p.lng;
    return origin;
}
...
function getPolylineLength(/* List <LatLong> */ gridPoints) {
    ...
    for (let i = 1, size = gridPoints.length; i <= size; i++) {
