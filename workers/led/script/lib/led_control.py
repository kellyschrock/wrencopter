...
FRONT_LEFT     	= range(8, 12)
FRONT_RIGHT    	= range(14, 18)
REAR           	= range(0, 8)
REAR_LEFT	   	= range(5, 8)
REAR_RIGHT     	= range(0, 4)
REAR_CENTER_TOP = range(0, 7)
REAR_CENTER_BTM = range(2, 5)
REAR_CENTER_SECTION	 = [0, 1, 2, 3, 4, 5, 6, 7]
HEADLIGHT		=  [12, 13]
TAILLIGHTS 			 = [18, 19, 20]
TAILLIGHTS_ALL		 = range(0, 7)
CYLONS				 = range(0, 20)
...
class LEDControl():
	...
	def disco(self):
		global _quit
		self.stopAnyThreads()
		self.task = threading.Thread(target = doCopMode, args=(self.strip, Color(255, 255, 0), Color(255, 0, 255), Color(80, 255, 80),))
		_quit = False
		self.task.start()
	...
	def rainbow(self):
		global _quit
		self.stopAnyThreads()
		self.task = threading.Thread(target = rainbow, args=(self.strip,))
		self.task.start()

	def chase(self, color):
		global _quit
		self.stopAnyThreads()
		self.task = threading.Thread(target = theaterChaseRainbow, args=(self.strip,))
		self.task.start()
...
def doCopMode(strip, white = Color(255, 255, 255), blue = Color(0, 0, 255), red = Color(255, 0, 0)):
	off = Color(0, 0, 0)
	...
	while True:
		...
		fastFlash(TAILLIGHTS, strip, white, shortDelay)
		fastFlash(HEADLIGHT, strip, white, shortDelay)

		fastFlash(REAR, strip, white, shortDelay)
		fastFlash(FRONT_LEFT, strip, blue, shortDelay)
		fastFlash(REAR, strip, red, shortDelay)
		fastFlash(FRONT_RIGHT, strip, white, shortDelay)
		fastFlash(TAILLIGHTS, strip, red, shortDelay)
		fastFlash(HEADLIGHT, strip, white, shortDelay)
		...
		fastFlash(TAILLIGHTS, strip, blue, shortDelay)
		fastFlash(HEADLIGHT, strip, white, shortDelay)
...
def doOnMode(strip, mode):
	...
	while True:
		if mode == "stabilize":
			allOn(strip, Color(255, 125, 0))
		elif mode == "land":
			allOn(strip, Color(255, 255, 255))
		elif mode == "acro":
			allOn(strip, Color(0, 255, 20))
		elif mode == "auto":
			doBreathe(strip, [200, 0, 100], [0, 0, 200], 1)
		elif mode == "circle":
			rainbowCycle(strip, 40, 10000) 
		elif mode == "rtl":
			doBlink(strip, Color(255, 0, 0))
		elif mode == "pos_hold":
			allOn(strip, Color(200, 0, 200))
		elif mode == "alt_hold":
			allOn(strip, Color(30, 255, 0))
		elif mode == "loiter":
			allOn(strip, Color(0, 220, 220))
		elif mode == "brake":
			brakeLights(strip)
		elif mode == "drift":
			...
			doBreathe(strip, [0, 200, 0], [200, 0, 200], 1)
			...
			doBlink(strip, Color(255, 0, 255), 500)
		elif mode == "manual":
...
def planeLights(strip, backColor=Color(255, 255, 255)):
	...
	while True:
		...
		fastFlash(TAILLIGHTS, strip, backColor, 20/1000.0)
...
def carLights(strip):
    ...
    for i in REAR:
        strip.setPixelColor(i, Color(160, 0, 0))
